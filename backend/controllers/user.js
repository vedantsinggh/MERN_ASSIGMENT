...
const acceptFriendRequest = TryCatch(async (req, res, next) => {
  ...
  if (request.receiver._id.toString() !== req.user.toString())
    return next(new ErrorHandler("You are not authorized to accept the Friend Request", 401));
...
const mutulFriends = TryCatch(async (req, res, next) => {
  const { userId } = req.body;
  const user = User3.findById(userId).populate("friends intrests");

  if (!user) {
    return next(new ErrorHandler("user doesn't exits", 404));
  }

  // if user exits then , find all the friends of the friends exclude user
  const friendsOfFriends = User3.find({
    _id: { $ne: userId }, // excluded the user itself
    friends: { $in: user.friends.map((friend) => friend._id) },
  }).populate("friends");
  // here we get all the friends object id with their Frinds array without userId

  if (!friendsOfFriends || friendsOfFriends.length == 0) {
    return next(new ErrorHandler("NO Mutual Friends Found"));
  }

  // Mapping through friendsOfFriends and calculating mutual friends
  let recommendations = friendsOfFriends.map((friend) => {
    const mutualFriendsCount = friend.friends.filter((f) => user.friends.includes(f._id)).length;
